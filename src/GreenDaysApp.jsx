  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', ...style }}>
      <svg viewBox="0 0 573 613" height="50%" width="auto" style={{ maxHeight: 104, color: 'var(--color-accent)', opacity: 0.5 }} aria-hidden="true">
        <path d={SPROUT_PATH} fill="currentColor" stroke="currentColor" />
      </svg>
    </div>
  );
}
// The same sprout glyph at a fixed size for empty states — muted (secondary
// ink via currentColor), a quiet placeholder rather than an active control.
function LeafMark({ size = 52 }) {
  return (
    <svg viewBox="0 0 573 613" height={size} width={size * 573 / 613} aria-hidden="true"
      style={{ display: 'inline-block', color: 'var(--color-text-tertiary)' }}>
      <path d={SPROUT_PATH} fill="currentColor" />
    </svg>
  );
}
    ...
    <div style={{ padding: '8px 20px 24px' }}>
      ...
      {list.length === 0 && (
        <div className="gd-card gd-card--muted" style={{ textAlign: 'center', padding: 28, marginTop: 4 }}>
          <div style={{ lineHeight: 0 }}><LeafMark size={44} /></div>
...
function KitchenShelf({ declared, prefs, basket, lang, country, onDeclare, onUndeclare, onAdd }) {
  ...
  const declare = (text) => {
    ...
    if (hit) { setProduceHit(decorate(hit, country)); return; }
    onDeclare(t);
    setDraft('');
  };
...
function ListScreen({ basket, checked, lang, country, prefs, declared, onDeclare, onUndeclare, onAdd, onRemove, onToggle, onOpen, onCook }) {
  const items = PRODUCE.filter((p) => basket[p.id] > 0).map((p) => decorate(p, country));
    ...
    <div style={{ padding: '8px 20px 24px' }}>
      ...
      {items.length === 0 && (
        <div className="gd-card gd-card--muted" style={{ textAlign: 'center', padding: 32, marginTop: 16 }}>
          <div style={{ lineHeight: 0 }}><LeafMark size={52} /></div>
...
function RecipeDetailScreen({ view, history, local, localData, onOpen, onSearchProduce, onClose, onGoHome, onTryAnother }) {
  ...
  const stars = r ? r.stars.map((id) => decorate(byId(id), rc)).filter(Boolean) : [];
  ...
  const shellStars = status === 'loading' ? (view.shellIds || []).map((id) => decorate(byId(id), rc)).filter(Boolean) : [];
  ...
  const freshProduce = (item) => {
    ...
    return decorate(stars.find((p) => t.includes(stripDia(p.name))) || PRODUCE.find((p) => t.includes(stripDia(p.name))), rc);
  };
